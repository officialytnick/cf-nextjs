...
const HtmlMinifier = () => {
  ...
  const handleMinify = () => {
    ...
    toast({ title: 'HTML Minified!', description: `Reduced by ${saved} characters (${Math.round(saved/input.length*100)}%)` });
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#202124] rounded-xl p-6 border border-gray-200 dark:border-gray-800 space-y-4">
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">HTML Input</label>
      ...
      <div className="flex gap-3">
        <Button onClick={handleMinify} className="bg-red-500 hover:bg-red-600 text-white">Minify HTML</Button>
      ...
      {output && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          ...
          {typeof window !== 'undefined' && (
            <DownloadPopup isOpen={showPopup} onClose={() => setShowPopup(false)} fileCount={1} onDownload={() => { performDownload(); setShowPopup(false); }} toolId="html-minifier" />
          )}
