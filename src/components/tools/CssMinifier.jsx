...
const CssMinifier = () => {
  ...
  const handleMinify = () => {
    ...
    toast({ title: 'CSS Minified!', description: `Reduced by ${saved} characters (${Math.round(saved/input.length*100)}%)` });
  };

  const copyOutput = () => {
    navigator.clipboard.writeText(output);
    toast({ title: 'Copied!', description: 'Minified CSS copied' });
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white dark:bg-[#202124] rounded-xl p-6 border border-gray-200 dark:border-gray-800 space-y-4">
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">CSS Input</label>
      ...
      <div className="flex gap-3">
        <Button onClick={handleMinify} className="bg-red-500 hover:bg-red-600 text-white">Minify CSS</Button>
      ...
      {output && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          ...
          {typeof window !== 'undefined' && (
            <DownloadPopup isOpen={showPopup} onClose={() => setShowPopup(false)} fileCount={1} onDownload={() => { performDownload(); setShowPopup(false); }} toolId="css-minifier" />
          )}
