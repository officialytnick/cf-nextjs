...
export default function FormatConverter({ tool }: any) {
  const { toast } = useToast();
  const rule = TOOL_RULES[tool?.id] || TOOL_RULES['image-converter'];
  const acceptString = rule.input.map((e: string) => '.' + e).join(',');

  const [files, setFiles] = useState<any[]>([]);
  const [outputFormat, setOutputFormat] = useState<string>(rule.userSelect ? 'webp' : rule.output);
  ...
  useEffect(() => {
    if (
      ...
      toast({ title: 'Conversion Complete', description: 'Opening download popup...' });

      setTimeout(() => setShowPopup(true), 1000);
    }
  }, [convertedCount, totalToConvert, toastShown, toast]);

  const handleFileSelect = useCallback(async (selected: File | File[]) => {
    const incoming = Array.isArray(selected) ? selected : [selected];

    if (files.length + incoming.length > MAX_FILES) {
      return toast({ title: 'Maximum reached', description: `You can upload up to ${MAX_FILES} images.`, variant: 'destructive' });
    }
    ...
    for (const file of incoming) {
      ...
      if (!rule.input.includes(ext)) {
        toast({ title: 'Invalid file type', description: `Accepted: ${rule.input.join(', ').toUpperCase()}`, variant: 'destructive' });
        continue;
      }
  ...
  const handleManualConvert = async () => {
    if (files.length === 0)
      return toast({ title: 'No images', description: 'Please upload images first.', variant: 'destructive' });
    ...
    setFiles((prev) => prev.map((f) => ({ ...f, loading: true, convertedBlob: null })));
  ...
  const handleDownloadAll = async () => {
    ...
    await downloadZip(
      ready.map((f) => ({ name: f.file.name.replace(/\.[^.]+$/, `.${ext}`), blob: f.convertedBlob })),
      'convertfreely.zip'
    );
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-xl p-6 border border-gray-200">
      <FileUploadZone onFileSelect={handleFileSelect} accept={acceptString} prompt={files.length === 0 ? 'Choose or drop images' : 'Add more images'} multiple />
      ...
      <div className="mt-6 md:flex md:items-end md:justify-between gap-6">
        <div className="md:flex-1">
          ...
          {rule.userSelect ? (
            ...
            <div className="px-4 py-3 border rounded-md bg-gray-50 text-sm">Output format: <strong>{rule.output.toUpperCase()}</strong></div>
          )}
        </div>

        <div className="mt-4 md:mt-0 md:w-1/3 space-y-3">
          <Button onClick={handleManualConvert} disabled={convertDisabled()} size="lg" className="w-full bg-red-500 hover:bg-red-600 text-white">
            Convert {files.length} Image{files.length > 1 ? 's' : ''}
          </Button>

          <Button onClick={() => setShowPopup(true)} disabled={!anyConverted} size="lg" className="w-full bg-green-600 hover:bg-green-700 text-white">
      ...
      <DownloadPopup isOpen={showPopup} onClose={() => setShowPopup(false)} fileCount={files.filter((f) => f.convertedBlob).length} onDownload={handleDownloadAll} />
    </motion.div>
  );
}
